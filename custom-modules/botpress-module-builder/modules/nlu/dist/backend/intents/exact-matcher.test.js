...
const intent1 = {
  ...
  contexts: ['global'],
  slot_definitions: [],
  utterances: [u1, u2].map(_utterance.makeTestUtterance)
};
const intent2 = {
  name: 'intent2',
  contexts: ['global', 'marijane'],
  slot_definitions: [],
  utterances: [(0, _utterance.makeTestUtterance)(u3)]
};
const noneIntent = {
  name: 'none',
  contexts: ['global'],
  slot_definitions: [],
  utterances: [(0, _utterance.makeTestUtterance)('lorem ipsum dolor sit amet')]
};
describe('Exact match', () => {
  const input = {
    intents: [intent1, intent2, noneIntent]
  };
  const exactMatchIndex = (0, _trainingPipeline.BuildExactMatchIndex)(input);
  describe('Build exact match index', () => {
    ...
    test('index contains proper keys', () => {
      const keys = [u1, u2, u3].map(u => u.replace(new RegExp(`(${_chars.SPECIAL_CHARSET.join('|')}|\\s)`, 'gi'), '').toLowerCase());
      expect(Object.keys(exactMatchIndex)).toEqual(keys);
    });
    test('index content', () => {
      const [k1, k2, k3] = [u1, u2, u3].map(u => u.replace(new RegExp(`(${_chars.SPECIAL_CHARSET.join('|')}|\\s)`, 'gi'), '').toLowerCase());
  ...
  test('find exact match', () => {
    const [utt1, utt2, utt3] = [u1, u3, 'This is just a test'].map(_utterance.makeTestUtterance);
    const pred1 = (0, _predictPipeline.findExactIntentForCtx)(exactMatchIndex, utt1, 'marijane');
    const pred2 = (0, _predictPipeline.findExactIntentForCtx)(exactMatchIndex, utt1, 'global');
    const pred3 = (0, _predictPipeline.findExactIntentForCtx)(exactMatchIndex, utt2, 'marijane');
    const pred4 = (0, _predictPipeline.findExactIntentForCtx)(exactMatchIndex, utt2, 'global');
    const pred5 = (0, _predictPipeline.findExactIntentForCtx)(exactMatchIndex, utt3, 'marijane');
    const pred6 = (0, _predictPipeline.findExactIntentForCtx)(exactMatchIndex, utt3, 'global');
...
//# sourceMappingURL=exact-matcher.test.js.map