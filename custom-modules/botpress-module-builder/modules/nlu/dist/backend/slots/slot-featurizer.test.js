...
describe('CRF Featurizer 2', () => {
  ...
  test('getWordWeight', () => {
    ...
    expect(featurizer.getWordWeight({ ...token,
      tfidf: 0
    }).value).toEqual('low');
    expect(featurizer.getWordWeight({ ...token,
      tfidf: 0.1
  ...
  test('getWordFeat', () => {
    const tokens = [{
      ...
      value: _tokenUtils.SPACE,
      isWord: false
    }].map(tok => Object.defineProperty(tok, 'toString', {
      value: jest.fn().mockReturnValue(tok.value)
    }));
  ...
  test('getInVocabFeat', () => {
    ...
    expect(featurizer.getInVocabFeat({ ...tokens[0],
      slots: ['lol.A.W']
    }, anIntent).value).toBeTruthy();
    expect(featurizer.getInVocabFeat(tokens[0], anIntent).value).toBeTruthy();
    expect(featurizer.getInVocabFeat({ ...tokens[1],
      slots: ['lol.A.W']
    }, anIntent).value).toBeFalsy();
    expect(featurizer.getInVocabFeat(tokens[1], anIntent).value).toBeFalsy();
    expect(featurizer.getInVocabFeat({ ...tokens[2],
      slots: ['lol.A.W']
    }, anIntent).value).toBeFalsy();
  ...
  test('getTokenQuartile', () => {
    ...
    expect(featurizer.getTokenQuartile(anUtterance, {
      index: 0
    }).value).toEqual(1);
    expect(featurizer.getTokenQuartile(anUtterance, {
      index: 1
...
//# sourceMappingURL=slot-featurizer.test.js.map