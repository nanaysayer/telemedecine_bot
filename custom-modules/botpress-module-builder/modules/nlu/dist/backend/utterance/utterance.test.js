...
var _utterance = _interopRequireDefault(require("./utterance"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const TOKENS = (0, _tokenUtils.tokenizeLatinTextForTests)('You might want to behave like if you are not like one of us , But you are !'); // split punct by space to simplify tokenization

const VECTORS = TOKENS.map(() => Array.from({
  length: 5
}, () => _lodash.default.random(0, 1, true)));
const POS_TAGS = ['PRON', 'VERB', 'VERB', 'PART', 'VERB', 'SCONJ', 'SCONJ', 'PRON', 'AUX', 'PART', 'SCONJ', 'NUM', 'ADP', 'PRON', 'PUNCT', 'CCONJ', 'PRON', 'AUX', 'PUNCT'].map(a => [a, '_']).reduce((tags, next) => tags.concat(next)).slice(0, -1);
describe('UtteranceClass', () => {
  describe('tokens', () => {
    test('Array is readonly', () => {
      const utterance = new _utterance.default(TOKENS, VECTORS, POS_TAGS, 'en');
    ...
    test('different tokens and vectors length throws', () => {
      expect(() => new _utterance.default(TOKENS.slice(0, -1), VECTORS, POS_TAGS, 'en')).toThrow();
    });
    test('toString', () => {
      const utterance = new _utterance.default(TOKENS, VECTORS, POS_TAGS, 'en');
      expect(utterance.tokens[0].toString()).toEqual('You');
      expect(utterance.tokens[0].toString({
        lowerCase: true
      })).toEqual('you');
      expect(utterance.tokens[1].toString()).toEqual(' ');
      expect(utterance.tokens[1].toString({
        realSpaces: true
      })).toEqual(' ');
      expect(utterance.tokens[1].toString({
        realSpaces: false
      })).toEqual(_tokenUtils.SPACE);
    });
    test('slots', () => {
      const utterance = new _utterance.default(TOKENS, VECTORS, POS_TAGS, 'en');
  ...
  describe('clone', () => {
    ...
    test('clone only', () => {
      ...
      _lodash.default.zip(utterance.tokens, u2.tokens).forEach(([t1, t2]) => {
        Object.entries(_lodash.default.omit(t1, 'slots', 'entities', 'toString')).forEach(([k, v]) => expect(v).toEqual(t2[k]));
      });
    });
    test('with entities', () => {
      ...
      _lodash.default.zip(utterance.tokens, u2.tokens).forEach(([t1, t2]) => {
        Object.entries(_lodash.default.omit(t1, 'slots', 'toString')).forEach(([k, v]) => expect(v).toEqual(t2[k]));
      });
    });
    test('with slots', () => {
      ...
      _lodash.default.zip(utterance.tokens, u2.tokens).forEach(([t1, t2]) => {
        Object.entries(_lodash.default.omit(t1, 'entities', 'toString')).forEach(([k, v]) => expect(v).toEqual(t2[k]));
      });
    });
    test('with entities and slots', () => {
      ...
      _lodash.default.zip(utterance.tokens, u2.tokens).forEach(([t1, t2]) => {
        Object.entries(_lodash.default.omit(t1, 'toString')).forEach(([k, v]) => expect(v).toEqual(t2[k]));
      });
    });
  });
  describe('toString', () => {
    const str = 'This IS a SUPerTest withFire'; //           0123456789012345678901234567

    const tokens = (0, _tokenUtils.tokenizeLatinTextForTests)(str);
    ...
    test('format options', () => {
      const u = new _utterance.default(tokens, fakeVectors, fakePOS, 'en');
      expect(u.toString(defaultOptions)).toEqual(str);
      expect(u.toString({ ...defaultOptions,
        lowerCase: true
      })).toEqual(str.toLowerCase());
      expect(u.toString({ ...defaultOptions,
        onlyWords: true
      })).toEqual(str.replace(/\s/g, ''));
      expect(u.toString({ ...defaultOptions,
        onlyWords: true,
        lowerCase: true
      })).toEqual(str.replace(/\s/g, '').toLowerCase());
    });
    test('slot options', () => {
      const u = new _utterance.default(tokens, fakeVectors, fakePOS, 'en');
      const slot = {
      ...
      expect(u.toString({ ...defaultOptions,
        slots: 'keep-name'
      })).toEqual(`This IS a ${slot.name} withFire`);
      expect(u.toString({ ...defaultOptions,
        slots: 'ignore'
      })).toEqual(`This IS a  withFire`);
    });
    test('entities options', () => {
      ...
      expect(u.toString({ ...defaultOptions,
        entities: 'keep-value'
      })).toEqual(`This IS a ${entity.value} withFire`);
      expect(u.toString({ ...defaultOptions,
        entities: 'keep-name'
      })).toEqual(`This IS a ${entity.type} withFire`);
      expect(u.toString({ ...defaultOptions,
        entities: 'ignore'
      })).toEqual(`This IS a  withFire`);
    });
  ...
  test('sentence embeddeing', () => {
    ...
    const u = new _utterance.default(testTokens, vecs, fakePOS, 'en');
...
const vecs = [[20.874107360839844, 13.226028442382812, 40.754878997802734, -32.03583526611328, 9.544014930725098, -18.946258544921875, -32.077903747558594, 25.72711944580078, 6.000054836273193, -9.980449676513672, -4.807291030883789, 34.21776580810547, -39.83692169189453, -41.74205780029297, -32.027469635009766, -3.2300822734832764, -8.309528350830078, 17.37891387939453, -25.159915924072266, -9.755718231201172, -21.471263885498047, 38.0972900390625, 19.229228973388672, -17.59087371826172, 13.534223556518555], [-0.24725811183452606, -10.69975471496582, 0.11991189420223236, 0.15629084408283234, 9.468283653259277, -2.0219221115112305, -5.423296928405762, 7.656525611877441, -6.705521583557129, 1.5514013767242432, 4.234777927398682, 3.0641133785247803, 3.788898229598999, -4.689056396484375, -2.73398756980896, -8.867668151855469, 7.824917316436768, -4.574779510498047, -9.939950942993164, -6.021678924560547, -0.011077036149799824, 4.60534143447876, 4.380100250244141, 0.2817268669605255, 0.825263500213623], [6.503832817077637, -9.005444526672363, -3.8556323051452637, 2.3468151092529297, 2.656834602355957, 6.978713512420654, -5.068508625030518, 6.288288116455078, -13.324929237365723, 2.51578950881958, 2.6000137329101562, 14.219351768493652, 5.4554643630981445, 5.286128044128418, -3.8965466022491455, -6.05485200881958, 11.869226455688477, -11.307221412658691, -1.5087023973464966, -8.646753311157227, 0.44821596145629883, 8.66211986541748, 8.303625106811523, 13.985923767089844, -0.19615978002548218], [4.50189208984375, -4.935824394226074, -0.042005181312561035, 2.3175597190856934, 4.412976264953613, -13.736227035522461, -13.990167617797852, 19.076614379882812, -3.8637607097625732, -2.875369071960449, 9.681108474731445, 14.190940856933594, -6.300146102905273, -2.748384475708008, -10.67545223236084, -15.081766128540039, -3.3130884170532227, 1.7095024585723877, -10.340690612792969, -2.966965675354004, -3.59036922454834, 13.048643112182617, 10.338794708251953, 12.132885932922363, 12.364285469055176], [4.14584493637085, -2.2263107299804688, 3.047293186187744, 3.635033130645752, 0.31205978989601135, 1.8614319562911987, -9.546428680419922, 5.078840732574463, 0.3299643397331238, 2.897230625152588, 1.3299314975738525, 0.7351471185684204, 5.249790191650391, -3.529052495956421, -3.4212493896484375, -5.099746227264404, 8.956042289733887, -2.9340271949768066, 0.8110866546630859, 2.8502562046051025, -4.7403388023376465, -4.358865261077881, 2.9119772911071777, 7.022226810455322, 2.365488290786743]];
const expectedEmbeddings = [0.1341326675190514, -0.06254902975677054, 0.16151883689646443, -0.09876129247260174, 0.11536730381631702, -0.09501117502566522, -0.25369285613049697, 0.24317262203117693, -0.07165771082652404, -0.019598536174092393, 0.0469875331544465, 0.2491950533824863, -0.11707818403666244, -0.1961129818560888, -0.20147780442964816, -0.14759219473910062, 0.07769623209882123, -0.0029155425803427614, -0.18939382527025744, -0.10210364508880357, -0.11266725049833885, 0.23113461596466228, 0.17243578054811445, 0.04260804160159498, 0.10276822801312079];
const globalTFIDF = {
...
//# sourceMappingURL=utterance.test.js.map