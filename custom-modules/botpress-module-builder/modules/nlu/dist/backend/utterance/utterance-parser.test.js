...
test('extract slots from utterance', () => {
  const utterance = 'My name is [Kanye](me) and your name is [Jay](you)'; //                .01234567890123456789012345678901234567890123456789.

  const extracted = (0, _utteranceParser.extractSlots)(utterance);
...
describe('parse utterance', () => {
  test('empty', () => {
    const res = (0, _utteranceParser.parseUtterance)('');
  ...
  test('no slots', () => {
    ...
    const res = (0, _utteranceParser.parseUtterance)(utterance);
  ...
  test('single slot', () => {
    const utterance = 'Brace yourself [Alex](you), big stuff coming.'; // raw            .01234567890123456789012345678901234567890123.
    ...
    expect(res.parsedSlots[0].rawPosition).toEqual({
      start: 15,
      end: 26
    });
    expect(res.parsedSlots[0].cleanPosition).toEqual({
      start: 15,
      end: 19
    });
  ...
  test('multiple slots', () => {
    const utterance = 'My name is [Kanye](me) and your name is [Jay](you)'; // raw            .01234567890123456789012345678901234567890123456789.
    ...
    expect(res.parsedSlots[1].rawPosition).toEqual({
      start: 40,
      end: 50
    });
    expect(res.parsedSlots[1].cleanPosition).toEqual({
      start: 34,
      end: 37
    });
  ...
  test('bounding spaces are trimmed from slots', () => {
    const utterance = 'My name is[ Kanye    ](me) and your name is[   Jay ](you).'; // raw            .012345678901234567890123456789012345678901234567890123.
...
//# sourceMappingURL=utterance-parser.test.js.map