...
describe('Extract Multiple', () => {
  ...
  let mockedFetch;
  beforeAll(() => {
    duck = new _duckling_extractor.DucklingEntityExtractor();
    mockedFetch = jest.spyOn(duck, '_fetchDuckling');
  });
  beforeEach(async () => {
    await _duckling_extractor.DucklingEntityExtractor.configure(true, ''); // reset mocked cache

    _duckling_extractor.DucklingEntityExtractor.enabled = true; // mock axios to remove this line
  });
  afterEach(() => {
    mockedFetch.mockReset();
  });
  test('When disabled returns empty array for each input', async () => {
    _duckling_extractor.DucklingEntityExtractor.enabled = false;
  ...
  test('calls extract with join char', async () => {
    ...
    expect(mockedFetch.mock.calls[0][0]).toContain(_duckling_extractor.JOIN_CHAR);
  });
  ...
  test('good results for each examples', async () => {
    const examples = ['one two three', 'nothing', 'now and then']; // expected        0123456789012____0123456____012345678901
  ...
  test('cache is not used when useCache is false', async () => {
    ...
    await duck.extractMultiple([ex], 'en', false);
    expect(mockedFetch).toHaveBeenCalledTimes(2); // make sure ex isn't removed from 2nd call
...
//# sourceMappingURL=duckling-extractor.test.js.map