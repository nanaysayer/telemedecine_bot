...
var _utteranceParser = require("../utterance/utterance-parser");

var _customEntityExtractor = require("./custom-entity-extractor");

var _validation = require("./validation");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const T = utterance => utterance.split(/( )/g);

const list_entities = [{
  entityName: 'fruit',
  fuzzyTolerance: _validation.FuzzyTolerance.Medium,
  ...
  entityName: 'airport',
  fuzzyTolerance: _validation.FuzzyTolerance.Loose,
  id: 'custom.list.city',
  languageCode: 'en',
  mappingsTokens: {
...
describe('list entity extractor', () => {
  test('Data structure test', async () => {
    ...
    const results = (0, _customEntityExtractor.extractListEntities)(utterance, list_entities, false);
  ...
  describe('exact match', () => {
    ...
    assertEntity('an [apple](qty:2 type:fruit confidence:0.90) can be a fruit but also [apple corporation](qty:2 type:company confidence:0.85)');
    ...
    assertEntity('[SF](qty:1 type:airport) is where I was born, I now live in [Quebec](qty:1 type:airport) [the city](qty:0)');
  });
  describe('fuzzy match', () => {
    ...
    describe('added chars', () => {
      ...
      assertEntity('[apple](qty:2) [coroporations](qty:1) [inc](qty:0)'); // too many added chars
    ...
    describe('bad keystrokes', () => {
      ...
      assertEntity('[poisoneouss blurberry](qty:1 value:Blueberry confidence:0.8) are berries that are blue'); // major

      assertEntity('[vluqberries](qty:0) are berries that are blue'); // assertEntity('[blumbeerries](qty:0) are berries that are blue') // this needs keyboard distance computation
      // assertEntity('[bluabarrias](qty:0) are berries that are blue') // this needs keyboard distance computation
      // minor letter reversal

      assertEntity('[blueebrries](qty:1 value:Blueberry) are berries that are blue'); // letter reversal + missing char
...
function assertEntity(expression) {
  const {
    utterance: text,
    parsedSlots
  } = (0, _utteranceParser.parseUtterance)(expression);
...
function textToUtterance(txt) {
  const tokens = T(txt);
  const vectors = tokens.map(() => new Array(100).fill(0));
  const posTags = tokens.map(_ => '_');
  return new _utterance.default(tokens, vectors, posTags, 'en');
}
//# sourceMappingURL=list-extractor.test.js.map