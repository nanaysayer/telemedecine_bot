...
export const IntentEditor: FC<Props> = props => {
  ...
  const handleUtterancesChange = (newUtterances: string[]) => {
    const l = intent.name.slice(-2)
    const newIntent = { ...intent, utterances: { ...intent.utterances, [l]: newUtterances } }
    saveIntent(newIntent)
  }

  const handleSlotsChange = (slots: NLU.SlotDefinition[], { operation, name, oldName }) => {
    let newUtterances = [...intent.utterances[l]]
    ...
    const newIntent = { ...intent, utterances: { ...intent.utterances, [l]: newUtterances }, slots }
    saveIntent(newIntent)
  }

  const l = intent.name.slice(-2)
  const utterances = (intent && intent.utterances[l]) || []
  return (
