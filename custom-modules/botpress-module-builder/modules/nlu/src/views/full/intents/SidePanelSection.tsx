...
export const IntentSidePanelSection: FC<Props> = props => {
  ...
  const createIntent = async (sanitizedName: string, rawName: string) => {
    const intentDef = {
      name: sanitizedName,
      utterances: { [sanitizedName.slice(-2)]: [rawName] } // note usage of raw name as first utterance
      // utterances: { [sanitizedName]: [rawName] } // note usage of raw name as first utterance
    }
